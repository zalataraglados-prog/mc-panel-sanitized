...
Security hardening: RCON is localhost-only by default, and public exposure requires explicit opt-in.
安全加固：RCON 默认仅本机可访问，公网暴露需显式开启。
Credential hardening: random RCON password + random panel secret key are generated by default, and execution logs now mask secret fields.
凭据加固：默认生成随机 RCON 密码和随机 panel secret_key，执行日志中对敏感字段脱敏。
Security baseline is now consistent across both planner-executor and legacy compose generation paths.
规划执行链路与 legacy compose 生成链路的安全默认值已对齐。
