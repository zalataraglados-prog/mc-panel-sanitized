# MC Panel 项目结构 (v1.0.3)

## 项目概览

MC Panel 是一个 Minecraft 部署前的配置裁决引擎，运行期面板为可选组件。项目主要由 Python 后端（FastAPI）、前端页面（Vite构建）以及 Python 部署 CLI 工具组成。在 v1.0.3 中，额外引入了材质包管理API、向导 Web 界面以及基于 React 的前端源码整理。
...
│   │   ├── players.py                 # 玩家管理 API
│   │   ├── rcon.py                    # RCON 通信 API
│   │   ├── resourcepacks.py           # 资源包/材质包管理 API (v1.0.3 新增)
│   │   ├── rules.py                   # 规则验证 API
│   │   ├── status.py                  # 服务器状态 API
│   │   ├── summary.py                 # 聚合信息 API
...
│   │   ├── resourcepacks.py           # 资源包/材质包处理服务 (v1.0.3 新增)
│   │   ├── server_properties.py       # server.properties 解析与处理 (v1.0.3 新增)
...
│   │
│   ├── wizard_ui/                     # 部署向导 Web UI 界面 (v1.0.3 新增)
│   │   ├── index.html                 # 向导主页面
│   │   └── wizard_script.js           # 向导交互逻辑
│   │
...
├── README.md                          # 库使用主说明文档
├── CHANGELOG.md                       # 更新日志
├── RELEASE_NOTES.md                   # 版本发布详细说明
├── RELEASE_DRAFT_v1.0.3.md            # v1.0.3 的发布草稿
└── .gitignore                         # Git 忽略规则
```
